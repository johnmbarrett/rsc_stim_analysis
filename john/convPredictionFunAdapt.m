function fun = convPredictionFunAdapt(kernelFun,lower,upper,isPlot,isFitToSummedData)
    ...
    function [deviation,predictions,kernel] = convPrediction(params,Bfull,Afull)
        ...
        for ii = 1:5
            for jj = 1:5
                ...
                normalizer=conv(squeeze(Bfull(ii,jj,:)),pastFilter);
               
                preds = max(0,conv2(squeeze(Bfull(ii,jj,:)),kernel')*params(2)-params(3));

                preds= params(5)*preds./(abs(normalizer)+params(5))+params(4);
                preds=interp1(1:length(preds),preds,(1:length(preds))+params(6))';

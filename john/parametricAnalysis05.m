function out = parametricAnalysis05(predictionsExp,predictionsAdapt)
...
if nargin < 1
    predictionsExp = arrayfun(@(~) nan(5,5,100),1:numel(dataset_list));
else
    if ~iscell(predictionsExp)
        predictionsExp = {predictionsExp};
    end

    assert(numel(predictionsExp) == numel(dataset_list));
end

if nargin < 2
    predictionsAdapt = arrayfun(@(~) nan(5,5,100),1:numel(dataset_list));
else
    if ~iscell(predictionsAdapt)
        predictionsAdapt = {predictionsAdapt};
    end

    assert(numel(predictionsAdapt) == numel(dataset_list));
end

predictions = [predictionsExp(:) predictionsAdapt(:)];

dataVsModelFigure = figure;
...
%         path1 = 'C:\_Data\Xiaojian\RSC-dmFC\';
        path1 = 'Z:\LACIE\DATA\Xiaojian\ephys\e-ephys\AnalyzedData\RSC-M2\';
        ...
        traces_summed = sum(traces(21:81, :, :), 1); % NB - only 0-50 msec post-stim
%         size(traces)
        ...
        prediction = permute(rot90(predictions{N,P},2),[2 1 3]); % need to reverse the transform from fitOptogeneticStimModels and account for the transpose on line 200
        
        predictionSummed = sum(prediction(:,:,21:80),3);
        
        % Plot the responses (raw) as a function of stim intensity:
        plotnum = plotnum + 1;
        subplot(plotrows, plotcols, plotnum)
        x = [.2 .4 .6 .8 1];
        h = plot(x, arrays_avgd);
        c = get(h,'Color');
        hold on;
        h = plot(x, predictionSummed, 'LineStyle', 'none', 'Marker', 'o');
        for ii = 1:numel(h)
            set(h(ii),'Color',c{ii});
        end
        set(gca, 'XLim', [0 1])
        ...
        c = get(h,'Color');
        hold on;
        h = plot(x, bsxfun(@rdivide,predictionSummed,max(predictionSummed,[],1)), 'LineStyle', 'none', 'Marker', 'o');
        for ii = 1:numel(h)
            set(h(ii),'Color',c{ii});
        end
        ...
        c = get(h,'Color');
        hold on;
        h = plot(x, bsxfun(@rdivide,predictionSummed,max(predictionSummed,[],2))', 'LineStyle', 'none', 'Marker', 'o');
        for ii = 1:numel(h)
            set(h(ii),'Color',c{ii});
        end
        ...
        % plot data and model vs intensity/duration
        
        figure(dataVsModelFigure);
        % Plot the data as a function of stim intensity:
        subplot(2, 4, 2*(P-1)+1)
        x = [.2 .4 .6 .8 1];
        plot(x, arrays_avgd);
        set(gca, 'XLim', [0 1])
        
        yy = [0 1.1*max(arrays_avgd(:))];
        
        ylim(yy);
        
        if P == 1
            text(gca,-0.25,yy(2)/2,'Data','FontSize',14,'HorizontalAlignment','center','Rotation',90)
        end
        
        text(gca,1.15,1.05*yy(2),sprintf('Stim in %s, probe in %s',stimSite,probeNames{P}),'FontSize',14,'HorizontalAlignment','center','VerticalAlignment','bottom');
        
        xlabel('Intensity'); ylabel('Response (events)');
        
        % Plot the data as a function of stim duration:
        subplot(2, 4, 2*(P-1)+2)
        x = [1 5 10 20 50];
        plot(x, arrays_avgd');
        set(gca, 'XLim', [0 50])
        ...
        % Plot the model as a function of stim intensity:
        subplot(2, 4, 2*(P-1)+5)
        x = [.2 .4 .6 .8 1];
        plot(x, predictionSummed);
        set(gca, 'XLim', [0 1])
        xlabel('Intensity'); ylabel('Response (events)');
        
        yy = [0 1.1*max(predictionSummed(:))];
        
        ylim(yy);
        
        if P == 1
            yy = ylim;
            text(gca,-0.25,yy(2)/2,'Model','FontSize',14,'HorizontalAlignment','center','Rotation',90)
        end
        
        % Plot the model as a function of stim duration:
        subplot(2, 4, 2*(P-1)+6)
        x = [1 5 10 20 50];
        plot(x, predictionSummed');
        set(gca, 'XLim', [0 50])
